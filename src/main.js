...
function setQuerySrc(src){
  ...
  u.search = src ? ('?src='+encodeURI(src)) : '';
...
glbInput.onchange = async () => {
  ...
  try{
    ...
    endOverlayOK();
    preset.value=''; updateShare(''); setQuerySrc('');
...
// 設 src 並等待（load 或 updateComplete 任一；30s 超時保險）
...
// === 覆蓋層控制：改用 style.display 強制開/關（不靠 hidden）===
function beginOverlay(msg,pct){ updateOverlay(pct,msg); overlay.style.display = 'flex'; }
function endOverlayOK(){
  updateOverlay(100,'完成');
  setTimeout(()=>{ overlay.style.display='none'; updateOverlay(0,''); }, 250);
}
function showError(e){
  overlay.style.display='flex';
  updateOverlay(0, e?.message ? `載入失敗：${e.message}` : '載入失敗，請檢查路徑或跨域 (CORS)');
}
